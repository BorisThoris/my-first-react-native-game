...
| **PLAY** | `PLAY-001`, `PLAY-002`, `PLAY-008`, `PLAY-009` | Rail order; chrome; tile FX intensity; pair markers gated |
...
| Namespace | IDs | Notes |
|-----------|-----|--------|
| **PLAY** | `PLAY-003`, `PLAY-004`, `PLAY-005` | Dual-row HUD IA; honest 5-life HUD + cap readout; floor shield escutcheon on hex badge |
...
| Namespace | IDs | Notes |
|-----------|-----|--------|
| **PLAY** | `PLAY-006` | CSS dais / torch ring / `stageBackdrop` polish (`GameScreen`) |
...
| Namespace | IDs | Notes |
|-----------|-----|--------|
| **PLAY / TASK-011** | `PLAY-007`, TASK-011 SVG | `authored-card-back.svg` / `authored-card-front.svg`; `TILE_TEXTURE_VERSION` 30; motif pivots in `cardArt/constants.ts` |
...
| Namespace | IDs | Notes |
|-----------|-----|--------|
| **Process** | `PLAY-010`, `QA-001`, **HUD-019** (checklist) | `CURRENT_VS_ENDPRODUCT.md` §2/§4/§6 sync; **`VISUAL_REVIEW.md`** HUD-019 / release checklist; `hud-inspect` + `visual-endproduct-parity` green |
...
## Narrative `TASK-00x` crosswalk (bookkeeping)

| TASK file | Relationship |
|-----------|----------------|
| TASK-004 | Overlaps `HUD-*`, `OVR-*`, `SIDE-*`; use namespaced IDs for status |
| TASK-011 | `PLAY-007`, card textures |
| TASK-013 | `HUD-010`, `PLAY-005` ornaments |
| TASK-014 / TASK-019 / TASK-020 | Process + captures; see **PLAY-010**, **QA-001** in [`README.md`](./README.md) |
| TASK-015 | Settings schema — overlaps **`PERF-001`**, reference controls |
| Remaining TASK-001–003, 005–010, etc. | Close when corresponding **`DS-*`**, **`META-*`**, **`PLAY-*`** rows close |
...
- **Ongoing process:** **`PLAY-010`** / **`QA-001`** — re-run `yarn test:e2e:visual:smoke`, `yarn test:e2e:renderer-qa`, and capture scripts when HUD/board/settings change; **`HUD-019`** — follow [`VISUAL_REVIEW.md`](../VISUAL_REVIEW.md) before release; optional commit of PNGs under `docs/reference-comparison/captures/` per policy.
