...
- **Engineering fixes (mobile visual):** (1) `startClassicRunFromModeSelect` used `toBeVisible` on the **sr-only** `Level 1` heading — switched to **`toBeAttached`** in `e2e/visualScreenHelpers.ts`. (2) Left-rail clicks were blocked by the HUD on mobile camera layout — **`z-index: 8`** on `.mobileCameraLeftToolbar` in `GameScreen.module.css` so the rail stacks above `.mobileCameraHud`. (3) Scenarios `01d` / `01e` use **90s** test timeout and **20s** region waits; inventory/codex use rail testids / labels (`GameLeftToolbar`, `GameScreen`).
