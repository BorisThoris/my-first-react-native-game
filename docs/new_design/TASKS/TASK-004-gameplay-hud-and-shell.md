...
- Gameplay shell, ornate HUD segments, left rail (inventory/codex via rail → overlay), and modal framing were brought toward `SCREEN_SPEC_GAMEPLAY.md`.
...
- **Mobile camera:** Left toolbar uses **`z-index: 8`** (`.mobileCameraLeftToolbar`) so rail controls stay above the HUD (`GameScreen.module.css`) — fixes Playwright `01d` / `01e` and real taps on small viewports.
