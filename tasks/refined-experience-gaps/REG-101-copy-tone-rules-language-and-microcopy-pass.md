...
## Problem
A **player-facing** **glossary**, **rules language**, and **microcopy** pass so HUD, Codex, and store strings **match** the rules engine — feeds `REG-064`, `REG-138`, and `REG-100` string keys.
...
- Align with `REG-100` and `REG-015`+ economy strings; `REG-057` / `REG-005` for progressive disclosure; **pseudo-loc** hooks per `REG-134`.
