...
## Problem
Run-**scoped** loadout, **stack** rules, and use limits for **consumables** in `RunState` are the backbone of the **inventory** data model — separate from the read-only `InventoryScreen` UI (`REG-094`, `REG-153`). The gap is **rules** (what you can change mid-run), not another mock panel.
...
- Spell mutability: what changes mid-run vs only at rest/shop; align with `REG-040` / `REG-117` and `REG-094` presentation.
- Cross-link `REG-015` / `REG-071` when consumables come from the shop; **offline**-only per `REG-052`.
