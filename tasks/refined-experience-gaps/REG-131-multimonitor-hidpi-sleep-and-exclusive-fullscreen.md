...
## Problem
Windows/Electron: DPI changes, sleep/resume, and monitor topology can break the renderer or `TileBoard` — extends `REG-031`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Topology matrix, resume from sleep, and scale factor events.
