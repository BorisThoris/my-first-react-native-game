...
## Problem
**Random event** rooms (branching text or choices) need a **data contract** and **RNG** bounds so runs are explainable and replayable per `REG-041`. Without it, “event” code becomes one-off `if` branches that break the map and softlock `REG-087`.
...
- Define event table schema, `runSeed` usage, and UI shell (modal vs map embed); test seeds in `game.test` where logic lives in `src/shared/`.
- Cross-link `REG-072` pacing, `REG-033` order, and `REG-150` if flip-adjacent effects appear.
