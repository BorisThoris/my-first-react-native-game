...
## Problem
Local clock changes, DST, and day boundaries can corrupt daily/weekly identity without an explicit **offline** policy — not an online service. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** UTC storage, day-key rules, and safe behavior on clock skew (see `REG-023`).
