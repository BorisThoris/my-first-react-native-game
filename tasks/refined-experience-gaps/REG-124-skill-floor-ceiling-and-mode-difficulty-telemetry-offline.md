...
## Problem
Per-mode skill band and **offline**-only playtest capture are underspecified; cannot rely on online aggregates. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Histograms, opt-in local logs, or hand playtest script tied to `REG-030` (privacy).
