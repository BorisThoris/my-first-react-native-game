...
## Problem
Central **asset** **placeholder** **inventory** and **drop-in** **contract** for all third-wave and ship slots — so `REG-113` marketing/bot work does not invent licensed art.
...
- Name placeholder paths, `REG-040` / `REG-030` bot constraints, and `REG-057` / `REG-005` store media deferrals (`REG-061`).
