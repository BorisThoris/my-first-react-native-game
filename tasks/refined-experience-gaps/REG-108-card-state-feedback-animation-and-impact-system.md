...
## Problem
**Card** **state** **feedback** **animation** and **impact** (match, miss, power) with `REG-110` / `REG-112` FX and motion budgets — the bridge between `REG-003` / `REG-031` board feel and performance caps.
...
- Tie to `card` / tile components and `REG-090`–`REG-104` frame pacing; `REG-080`–`REG-100` overlay timing if combined.
