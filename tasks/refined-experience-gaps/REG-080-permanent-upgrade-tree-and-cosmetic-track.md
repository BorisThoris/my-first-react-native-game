...
## Problem
Permanent **meta** upgrades and **cosmetic** tracks need scope, gating, and how they sit beside run-scoped power — without silently becoming pay-to-skip. This ticket makes the **permanent** layer explicit for `SaveData` and UI.
...
- Distinguish **meta** currency vs run currency; document migration in `REG-089` if schema moves.
- Align with `REG-016` and `REG-115` for feature flags; **online** not required in v1 (`REG-052`).
