...
## Problem
Overlay, offline Steam mode, and app id behavior need explicit edge cases without introducing online services — `REG-060`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Offline Steam, overlay hooks, and failure mocks.
...
- **Slots (planning):** legal / credits / license text and any support screenshots for Steam or docs may need final sign-off.
- **Fallbacks (allowed):** placeholder copy, draft credits, in-app shell only. No bot-generated final legal or licensed store media.
- **Bot constraint:** do not generate shippable legal text; legal owner reviews `REG-116` outputs.
