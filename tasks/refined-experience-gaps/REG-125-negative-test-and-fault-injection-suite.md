...
## Problem
Happy-path tests dominate; disk full, IPC failure, and GPU loss need scripted negative cases tied to `REG-040` and `REG-057`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Fault injection list and automation hooks where safe.
