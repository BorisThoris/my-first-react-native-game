...
## Problem
**Input** **latency** **frame** **pacing** and **responsiveness** (tap-to-flip, modal open) — P0 for **refined** feel on `REG-002` / `REG-004` shell and `REG-020` / `REG-028` WebGL path.
...
- Map to rAF, `useAppStore` update batching, and `REG-080`–`REG-100` overlay; `REG-001` / `REG-100` touch vs pointer.
