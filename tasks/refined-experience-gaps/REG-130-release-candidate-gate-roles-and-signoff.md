...
## Problem
RC0/RC1 need explicit “who signs what” and no-ship rules — **offline**-scoped gates only. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** RACI-style signoff, `REG-119` handoff, and P0 blockers from combinatoric/neg tests when product says yes.
