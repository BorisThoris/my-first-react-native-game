...
## Problem
Battery/thermal throttling and integrated GPU can force worse presets; `REG-109` needs explicit degradations and messaging. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Power-state matrix vs quality preset.
