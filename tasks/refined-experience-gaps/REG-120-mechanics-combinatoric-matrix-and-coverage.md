...
- This file (REG-120) — *combinatoric matrix anchor; fourth wave*
...
## Problem
Relic, mutator, mode, and board-size combinations are not fully enumerated for test coverage; rare stacks can diverge from design intent in `game.ts` and the mechanics catalog. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Combinatoric matrix, priority tiers, and traceability to `GAMEPLAY_MECHANICS_CATALOG.md`.
...
- `REG-148-hazard-and-trap-vocabulary.md` — *Fifth wave: core gameplay / hazard combinatorics*
