...
## Problem
Length expansion and RTL-shaped pseudo-loc catch clipped HUD and modals before real L10n — `REG-055` / `REG-100`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Pseudo build flag, spot-check list, and overflow acceptance.
