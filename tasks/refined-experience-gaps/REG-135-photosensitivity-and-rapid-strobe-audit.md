...
## Problem
Board and FX can exceed safe flash frequency for a subset of players; needs audit beyond reduce-motion only — `REG-112`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Strobe pass, cap policy, and reduce-motion path.
