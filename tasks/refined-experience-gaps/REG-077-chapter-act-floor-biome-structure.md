...
## Problem
Chapter, **act**, **floor** progression, and **biome** theming need a **stable mental model** (names, order, gating) that matches the endless **schedule** and `REG-120` combinatoric coverage. Drift here makes Codex and results screens lie.
...
- Document chapter boundaries vs `FLOOR_SCHEDULE_RULES_VERSION`; keep `mechanics-encyclopedia` in sync for `REG-138`.
- Cross-link `REG-082` (quests), `REG-083` dailies, and `REG-069` map progression.
