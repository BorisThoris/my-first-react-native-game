...
## Problem
Relic **offer** flows — **reroll**, **ban**, **upgrade** at shops or events — need explicit costs, availability, and failure UI so `REG-071` offers and `REG-019` build fantasy align. “Reroll” must not be a hidden debug button.
...
- State costs in run currency, charges, and per-floor limits; store in or derive from `RunState` with clear `GAME_RULES_VERSION` notes.
- Cross-link `REG-015`, `REG-070`, `REG-152` for placement on the run.
