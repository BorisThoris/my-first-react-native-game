...
## Problem
UI copy, Codex, and the rules engine can disagree; forbidden relic/mutator stacks need a single source of truth order. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Drift check script or checklist vs `mechanics-encyclopedia` and `REG-089`.
