...
## Problem
Long sessions can expose leaks and timer drift; needs a soak matrix beyond spot checks (extends `REG-110`). **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Multi-hour play profile, mode rotation, and memory/GPU watch.
