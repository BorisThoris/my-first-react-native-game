...
## Problem
**Codex** as a final **knowledge base** (search, links, depth per topic) for mechanics and lore — the ship-quality companion to `mechanics-encyclopedia` and `REG-138` anti-drift work.
...
- Keep Codex in sync with `mechanics-encyclopedia` and `REG-094` navigation; `REG-100` for loading.
- If export/share exists, `REG-040` / `REG-128` for redaction of saves from examples.
