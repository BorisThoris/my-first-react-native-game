...
## Problem
When `GAME_RULES_VERSION` or economy changes, you need **baseline** scores or sim outputs to detect unintended drift — `REG-086` depth. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Stored baselines, diff tolerances, and when to re-record.
