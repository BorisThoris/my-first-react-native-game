...
- This file (REG-147) — *local trust / no online anticheat posture (bookend)*
- `tasks/refined-experience-gaps/REG-119-bot-batch-plan-and-product-acceptance-report.md`

## Problem
Document why **server** anticheat and captcha are out of scope for an offline/premium v1; local integrity only. Aligns with `REG-123` and README **online** scope bar. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Design note for PR, support, and `REG-119` acceptance — no new online surface.
