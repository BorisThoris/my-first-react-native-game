...
## Problem
The **inventory** / run-prep **UI** (read-only `InventoryScreen` + future consumable rows) must match **`REG-079`** invariants and **Choose Path** polish — this ticket is the **screen**; `REG-079` and `REG-153` are the **data** contract.
...
- See `src/renderer/components/InventoryScreen.tsx` and `REG-079` / `REG-153`; keep **offline**; cross-link `REG-013` and `REG-010` for entry points.
