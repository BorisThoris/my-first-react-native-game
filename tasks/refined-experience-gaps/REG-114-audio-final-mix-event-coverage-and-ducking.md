...
## Problem
Final **audio** **mix** **event** **coverage** and **ducking** — the ship bar for SFX/Music/VO vs `REG-003` / `REG-031` gameplay (`gameSfx`, menu, run).
...
- Tie to `src/renderer/audio/`, `REG-003` / `REG-031` identity docs, and `REG-040` / `REG-030` Bluetooth / route edge (`REG-142`).
