...
## Problem
**Empty**, **loading**, **error**, and **locked** states across meta and gameplay must be **on-brand** and **actionable** — the polish layer on top of `REG-090`–`REG-104` shell goals so players never see dead `Panel`s with no CTA.
...
- Per-screen pass list tying to `REG-101` (copy) and `REG-090` quality bar; **offline** error copy must not assume network (`REG-052`).
