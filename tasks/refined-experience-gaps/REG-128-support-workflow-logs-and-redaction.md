...
## Problem
Support needs a clear workflow for what the player can attach (save export) vs what must be stripped — no accidental `SaveData` secrets in public tickets. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Redaction checklist and template for internal triage only.
