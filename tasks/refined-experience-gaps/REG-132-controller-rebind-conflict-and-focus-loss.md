...
## Problem
Gamepad, keyboard, and focus transitions can double-fire or steal input — `REG-029` needs an edge matrix. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Hotplug, focus blur, and conflict resolution rules.
