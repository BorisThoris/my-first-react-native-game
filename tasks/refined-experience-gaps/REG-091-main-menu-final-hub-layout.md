...
## Problem
The **main menu** as a final **hub** (mode entry, continue run, profile-adjacent strip per product) must hit **Choose Path** density and **navigation** clarity — the anchor for return-from-run flows and `REG-099` back stack.
...
- Tie to `REG-010`, `REG-013`, and `REG-060` / `REG-061` for Steam; cross-link `REG-100` and `REG-035` where scope touches social strip decisions.
