...
## Problem
Crashes need **local** categorization and optional user export without server APM or PII; aligns with `REG-063`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Buck names, user opt-in, and redaction story.
