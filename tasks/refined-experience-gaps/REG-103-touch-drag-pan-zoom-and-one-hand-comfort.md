...
## Problem
**Touch** **drag** **pan** **zoom** and **one-hand** comfort for board and shell — the gameplay complement to `REG-102` and `REG-001` / `REG-100` HUD work on phones.
...
- Link `TileBoard` and gesture handlers; `REG-090`–`REG-104` input latency and `REG-029` / `REG-006` for hit targets and a11y.
