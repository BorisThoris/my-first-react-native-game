...
## Problem
Run export, replay, and in-session RNG can drift with version bumps or missed seed plumbing — breaks trust in `RunExportPayload` and daily seeds. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Determinism audit, `GAME_RULES_VERSION` coupling, and regression when RNG paths change.
