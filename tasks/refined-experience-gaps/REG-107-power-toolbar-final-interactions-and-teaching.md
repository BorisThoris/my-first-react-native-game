...
## Problem
**Power** **toolbar** **interactions** and **teaching** (verbs, costs, why disabled) so `REG-002` / `REG-004` teach loop lands in product — not discoverability by accident.
...
- Connect to `REG-003` / `REG-031` power rules and `REG-002` / `REG-004` guard/combo; `REG-090`–`REG-104` latency for button response.
