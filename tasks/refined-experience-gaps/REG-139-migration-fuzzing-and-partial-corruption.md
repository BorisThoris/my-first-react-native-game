...
## Problem
Upgrades that touch `SAVE_SCHEMA_VERSION` can fail on **partial** files or merged junk — `REG-040` / `REG-117` need fuzz thinking. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Corruption fixtures and recovery acceptance.
