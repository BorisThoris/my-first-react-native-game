...
## Problem
Catalog rows, symbol assets, and puzzles can orphan when IDs drift — needs a completeness scan vs `mechanics-encyclopedia` / `game-catalog`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Orphan report and ownership for missing data.
