...
## Problem
Multi-monitor and restored bounds can break layout or put the window off-screen — Electron/window `IPC` expectations. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Save/restore, clamp to visible area, and DPI.
