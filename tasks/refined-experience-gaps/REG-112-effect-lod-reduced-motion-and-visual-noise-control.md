...
## Problem
**Effect** **LOD** **reduced** **motion** and **visual** **noise** control — a11y and comfort bridge to `REG-112` flash audit (`REG-135`) and `REG-001` / `REG-100` HUD busy-ness.
...
- Tie to settings flags and `REG-040` / `REG-030` photosensitivity; `REG-040` / `REG-030` hazard tile `REG-160` when effects are gameplay-critical.
