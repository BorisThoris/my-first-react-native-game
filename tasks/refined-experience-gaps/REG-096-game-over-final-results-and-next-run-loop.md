...
## Problem
**Game over** and **next run** loop (results density, **run it back**, and emotional clarity) is the high-traffic close of every session — must match `REG-070` / `REG-130` mobile above-fold and `REG-019`+ build recap expectations.
...
- Tie to `REG-032`, `REG-100`, and `REG-013` (mode art); `REG-030` for optional local run stats, **not** online boards.
