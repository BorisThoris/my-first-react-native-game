...
## Problem
**Quests**, **contracts**, and a **campaign ladder** need authored steps, progress storage, and failure/retry that match `RunState` and run summary. Without a ladder spec, `REG-021` and meta screens cannot be verified.
...
- Define contract state machine, UI surfaces (log/journal in `REG-085` direction), and version bumps when quest rules change.
- Tie to `REG-017` and `REG-023` (daily) only where product wants overlap; **offline** first.
