...
## Problem
Vendor **stock**, **pricing** curves, and **reroll** affordances must be specified against `REG-015`’s run currency so players see *why* an offer is cheap, expensive, sold out, or re-rollable. The gap is end-to-end **economy behavior**, not a one-line “shop exists.”
...
- Bind stock/pricing to `RunState` currency fields; document reroll costs and failure UX (broke, at cap).
- Defer base currency design to `REG-015` / `REG-024` — this ticket is vendor **dynamics** and display honesty.
- Coordinate with `REG-152` (shop placement on the map) when the graph lands.
- Preserve `REG-052` / **offline** bar; no server-backed store catalog in v1.
