...
## Problem
**Treasure**, **secret** rooms, and **bonus** rewards need discoverability rules, eligibility, and anti-grind limits so they feel special without breaking economy (`REG-072`) or `REG-110` performance on large effects.
...
- Specify spawn conditions, one-time flags in `RunState`/`SaveData`, and how rewards show in the HUD and run summary.
- Align with `REG-115` / `REG-118` for demo vs full content flags.
