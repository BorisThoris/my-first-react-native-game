...
## Problem
**Balance** simulation, **economy** drift detection, and **drop-rate** tuning need baselines, scripts or harnesses, and `REG-141`-style snap comparisons — not only manual feel. Informs `REG-072` and `REG-019` / `REG-156` exploit review.
...
- Point to `REG-030`, `REG-086` adjacent docs, and `docs/gameplay-tasks` balance notes; `GAME_RULES_VERSION` on rule changes.
- **Offline**-only sim inputs; `REG-052` for any future server truth.
