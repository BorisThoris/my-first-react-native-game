...
## Problem
**Performance** **budget** and **quality** **preset** **enforcement** (mobile vs desktop, thermal throttling) — makes `REG-028` device grid honest under load (`REG-090`–`REG-104` GPU).
...
- Point to `REG-090`–`REG-104` presets, `REG-003` / `REG-031` WebGL, and `REG-002` / `REG-004` laptop power; `REG-040` / `REG-030` crash buckets if OOM in field.
