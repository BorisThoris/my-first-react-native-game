...
## Problem
Output device change, downmix, and Bluetooth delay can desync SFX; needs edge matrix — `REG-114` / `REG-037`. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Device hot-swap, latency policy, and mix validation.
