...
## Problem
Beyond ad hoc a11y: a formal per-screen WCAG 2.2 test matrix and screen reader where applicable — `REG-029` / `REG-056` depth. **Enterprise depth wave:** this ticket adds explicit combinatoric, process, and edge coverage that large teams use so gaps are not left only in `GP-*` or implicit QA habit.
...
**Focus:** Test matrix, documented exceptions, and automation-friendly selectors.
