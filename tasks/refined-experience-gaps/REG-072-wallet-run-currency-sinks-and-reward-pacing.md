...
## Problem
Run **currency** in, **sinks** (shop, rest, services), and **reward pacing** per floor/act must be expressed as numbers or curves QA can test — not only narrative “you earn favor.” Imbalanced sinks block `REG-015` and `REG-086` (balance sim) from meaning anything.
...
- Tie earn/spend to `RunState` and document conversion at run end; cross-link `REG-024` unification and `REG-086` baselines (`REG-141`).
- Sequence with `REG-033` and P0 economy gates from `REG-000` before broad meta sinks.
- Optional **offline** playtest capture per `REG-030` / `REG-124` — not server aggregates.
