...
## Problem
**Collection** as a final **reward gallery** (browsable, legible, satisfying) with honest empty states — the meta surface that sells progression (`REG-011` direction) at ship quality.
...
- Connect to `SaveData` unlock bits and `REG-016` if meta tree ties in; `REG-100` and `REG-015` for placeholder art per `REG-113`.
