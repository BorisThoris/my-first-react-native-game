...
## Problem
Rolling **Choose Path**-level **quality** to every path and sub-mode requires a **definition of “done per screen**” and a **rollout** order — the hub ticket for the third-wave **shell** program under `REG-068`’s bar.
...
- Coordinate with `REG-013` (menu art), `REG-010` (discoverability), and per-screen `REG-091`+ items; `REG-102`–`REG-104` for layout grid.
- No **online** requirements; `REG-052` for deferred social/leaderboard surfaces.
