/**
 * Walks docs/visual-capture/<device>/{portrait,landscape}/ for PNGs and writes AUDIT.md + INVENTORY.md.
 */
/* eslint-env node */
...
const CAPTURE_ROOT = join(REPO_ROOT, 'docs', 'visual-capture');
...
function buildAuditMarkdown({ deviceId, orientation, pngFiles, relDir }) {
    ...
    sections.push('');
    if (rows.length === 0) {
        sections.push('_No PNGs yet. Run `yarn capture:visual-inventory` from the repo root._');
    } else {
...
function main() {
    ...
    for (const deviceId of listDeviceDirs()) {
        for (const orientation of orientations) {
            ...
            const relDir = join('docs', 'visual-capture', deviceId, orientation).replace(/\\/g, '/');
    ...
    process.stdout.write(`Wrote ${join('docs', 'visual-capture', 'INVENTORY.md')}\n`);
